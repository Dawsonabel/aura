/* Dev-only: make some classmates pick you, so there's aura to look at.
   ...
   The clue-reveal screen can't be exercised without real auras, and auras only exist when *other*
   ...
   Usage:  pnpm --filter api seed-auras <username> [count]
   e.g.    pnpm --filter api seed-auras tonyhawk 6
...
async function main() {
  ...
  if (!username) throw new Error('Usage: pnpm --filter api seed-auras <username> [count]');
  ...
  console.log(`\nSeeded ${made} auras for @${username}. They now have ${n} in total.`);
}

main().catch(e => {
  console.error('seed-auras failed:', e);
  process.exit(1);
});
