...
const yoga = createYoga<{ req: Request; env: Env; ip: string; waitUntil: (p: Promise<unknown>) => void }>({
  ...
  context: async ({ req, env, ip, waitUntil }): Promise<GraphQLContext> => {
    ...
    return {
      ...
      ip,
      db,
      ratelimit: makeRateLimiter(env.UPSTASH_REDIS_REST_URL, env.UPSTASH_REDIS_REST_TOKEN, env.REDIS_KEY_PREFIX),
      rounds: makeRoundStore(env.UPSTASH_REDIS_REST_URL, env.UPSTASH_REDIS_REST_TOKEN, env.REDIS_KEY_PREFIX),
      me,
      isAdmin,
      waitUntil,
