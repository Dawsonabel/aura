...
/* 13A's "what you've won" chips: the prompts someone has been picked for, most-picked first.
   ...
   Scoped to the same 30-day window the Inbox uses, so a chip can't outlive the flames behind it —
   a trophy for a prompt whose flames have all expired would be a number with nothing to back it. */
...
export async function superlativesFor(db: Db, userId: string, tuning: Tuning): Promise<Superlative[]> {
  const since = new Date(Date.now() - tuning.flameLifetimeDays * 86400_000).toISOString();
  const rows = await db.getFlameCountsByPoll(userId, since);
