...
import { useMarkAurasRead } from './useMarkAurasRead';
...
describe('useMarkAurasRead', () => {
  test('calls the mutation with the Clerk token', async () => {
    gqlFetchMock.mockResolvedValue({ markAurasRead: true });
    const { result } = renderHook(() => useMarkAurasRead(), { wrapper });

    result.current.mutate();
    await waitFor(() => expect(result.current.isSuccess).toBe(true));

    expect(gqlFetchMock).toHaveBeenCalledWith(expect.stringContaining('mutation MarkAurasRead'), undefined, 'a-real-token');
  });
});
