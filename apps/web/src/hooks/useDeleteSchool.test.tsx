...
describe('useDeleteSchool', () => {
  test('sends the school id', async () => {
    gqlFetchMock.mockResolvedValue({ deleteSchool: true });
    const { result } = renderHook(() => useDeleteSchool(), { wrapper });

    result.current.mutate('sch_1');
    await waitFor(() => expect(result.current.isSuccess).toBe(true));

    expect(gqlFetchMock).toHaveBeenCalledWith(expect.stringContaining('mutation DeleteSchool'), { id: 'sch_1' }, 'a-real-token');
  });
});
