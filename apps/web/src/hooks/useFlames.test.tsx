...
import { useFlames } from './useFlames';
...
describe('useFlames', () => {
  test('fetches flames with the Clerk token when signed in', async () => {
    useAuthMock.mockReturnValue({ getToken: vi.fn().mockResolvedValue('a-real-token'), isSignedIn: true });
    const result_ = { flames: [], coins: 2, godMode: false, bonusRevealsLeft: 0 };
    gqlFetchMock.mockResolvedValue({ flames: result_ });

    const { result } = renderHook(() => useFlames(), { wrapper });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));

    expect(result.current.data).toEqual(result_);
    expect(gqlFetchMock).toHaveBeenCalledWith(expect.stringContaining('query Flames'), undefined, 'a-real-token');
  });
